...
const InquiryTable = () => {
  ...
  const fethcuser = async () => {
    const response = await fetch(`${baseurl}/api/auth/getallinquiry`, {
      headers: { Authorization: `Bearer ${token}`,
    'x-branch-id': selectedbranchid,
   },
    });
    if (!response.ok) throw new Error('Error fetching inquiry');
    return response.json();
  };

  // Fetch bookings
  const { data, isLoading, isError } = useQuery('inquiry', fethcuser);

  // Delete booking mutation
  const deleteInquiryMutation = useMutation(
    async (id) => {
      const response = await fetch(`${baseurl}/api/auth/inquiry/delete/${id}`, {
        method: 'DELETE',
        headers: { Authorization: `Bearer ${token}`,
        'x-branch-id': selectedbranchid,
         },
  ...
  const handleAddInquiry = () => {
    navigate(`/inquiry/add`);
  };

  const handledeleteInquiry = (id) => {
    if (window.confirm('Are you sure you want to delete this booking?')) {
      deleteInquiryMutation.mutate(id); // Call mutation to delete the booking
    }
  };
              ...
              <tbody>
                {data?.map((inquiry, index) => (
                  <tr key={inquiry.id}>
                    ...
                    <td>{inquiry.customer_phoneno}</td>
                    <td>{inquiry.no_of_pax}</td>
                    <td className=''>{inquiry.remarks}</td>  
                    <td className={inquiry.status}>{inquiry.status}</td>
                    <td>
                      ...
                      >
                        {deleteInquiryMutation.isLoading ? (
                          <svg
