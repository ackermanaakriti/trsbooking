...
const Roomtable = () => {
  ...
  const fetchRooms = async () => {
    const response = await fetch(`${baseurl}/api/auth/getallrooms`, {
      headers: { 
        'x-branch-id': selectedbranchid,
       },
    });
    if (!response.ok) throw new Error('Error fetching rooms');
    return response.json(); // Assumes response is an array of rooms
  };

  // Fetch rooms using React Query
  const { data: roomsData, isLoading, isError } = useQuery('room', fetchRooms);

  // Mutation for deleting a room
  const deleteRoomMutation = useMutation(
    async (id) => {
      const response = await fetch(`${baseurl}/api/auth/room/delete/${id}`, {
        method: 'DELETE',
        headers: { 'x-branch-id': selectedbranchid, },
      });
      if (!response.ok) throw new Error('Error deleting room');
      return id; // Return room id after deletion
    },
    {
      onSuccess: (id) => {
  ...
  const handleDeleteBooking = (id) => {
    if (window.confirm('Are you sure you want to delete this booking?')) {
      deleteRoomMutation.mutate(id); // Call mutation to delete the room
    }
  };
  ...
  return (
    <>
    {isLoading ? <Loading/> :  <div className="main-container-wrapper">
      <div className="main-container-inner">
        <div className="main-header-wrapper">
          <div className="main-header-wrapper-inner">
            {user?.role == 'admin'?
                        <button className="button" onClick={handleAddBooking}>Add Room</button>
                        :'' }
              ...
              <tbody>
                {roomsData?.map((room, index) => (
                  <tr key={room.id}>
                    ...
                    <div>
                    <td>
                      <button onClick={() => editBooking(room.id)}>
                      ...
                      <button
                        onClick={() => handleDeleteBooking(room.id)}
