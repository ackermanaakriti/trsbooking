...
import { useParams, useHistory } from 'react-router-dom'; // For reading URL params and redirection
...
const EditRegister = () => {
  ...
  // Fetch user data on component mount (for editing)
  useEffect(() => {
    const fetchUserData = async () => {
      try {
         const response = await fetch(`${baseurl}/api/auth/getuserdata/${id}`, {
  ...
  // Handle the form submission (update user)
  const handleUpdate = async (e) => {
    e.preventDefault();
    try {
      const updatedUser = { name, email, password, role };

      const response = await axios.put(`${baseurl}/api/auth/updateusers/${id}`,{
        updatedUser, 
        header:{
